...
void jackknife_analysis()
{
    ...
    for (int time = 0; time < nTimes; time++)
    {
        for (int j = 0; j < nFiles; j++)
        {
            ...
            C3_jth_RFunc[time][j] = 0;
            C3_jth_IFunc[time][j] = 0;
            ...
            for (int file = 0; file < nFiles; file++)
            {
                if (j != file)
                {
                    ...
                    C3_jth_RFunc[time][j] += C3_RFunc_time_file[time][file];
                    C3_jth_IFunc[time][j] += C3_IFunc_time_file[time][file];
                }
            }

            C2_jth_RFunc[time][j] = C2_jth_RFunc[time][j] / Float_t(nFiles -1);
            C2_jth_IFunc[time][j] = C2_jth_IFunc[time][j] / Float_t(nFiles -1);
            C3_jth_RFunc[time][j] = C3_jth_RFunc[time][j] / Float_t(nFiles -1);
            C3_jth_IFunc[time][j] = C3_jth_IFunc[time][j] / Float_t(nFiles -1);

            jth_C2_hist_real[j]->SetBinContent(time+1, C2_jth_RFunc[time][j]);
            jth_C2_hist_imag[j]->SetBinContent(time+1, C2_jth_IFunc[time][j]);
            jth_C3_hist_real[j]->SetBinContent(time+1, C3_jth_RFunc[time][j]);
            jth_C3_hist_imag[j]->SetBinContent(time+1, C3_jth_IFunc[time][j]);
        }
    }
